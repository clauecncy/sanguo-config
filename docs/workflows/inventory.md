...
3. 按名称和版本匹配武将，按名称匹配战法。必要时通过公共库定向查询；截图不清晰的字段询问用户，不猜填。若确认是公共库缺失的新实体，先按公共数据工作流补入截图明确的身份和字段、来源索引、SQL 基线并独立重建校验，再写个人库存。赛季、效果、品质等未显示字段保持未知；不能因公共库缺项而默认写成无实体 ID 的待补录库存。若证据不足以确认身份，暂停该项并向用户核对。
...
只统计金将和金紫战法，范围外条目不进入当前库存。新增条目必须先有经核准的公共实体 ID；更新入口遇到公共库缺项即拒绝写入，不能伪造 ID，也不能以无 ID 的待补录项作为完成状态。`validate` 对旧数据中的未解析项仍显式报告，便于后续纠正。
